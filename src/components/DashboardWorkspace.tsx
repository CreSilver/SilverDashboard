...
function createWidgetDefaultData(type: WidgetType, title: string): DashboardWidget {
  ...
  switch (type) {
    ...
    // 🚀 NOVINKA: Výchozí konfigurační data pro nový Seznam / ToDo
    case 'list':
      return { 
        id, 
        type: 'list', 
        title, 
        gridSize: '2x2', 
        isPinnedToSummary: false, 
        data: { mode: 'todo', listStyle: 'bullet', showPercentage: true, items: [] } 
      };
...
// 🚀 REGISTR WIDGETŮ REGISTRUJE I 'list'
...
function useDashboardState(initialData: Dashboard) {
  ...
  return {
    dashboard, loading, isDropdownOpen, isEditingTitle, titleInput, activeMenuId, isEditMode,
    setTitleInput, setIsDropdownOpen, setIsEditingTitle, setActiveMenuId, setIsEditMode,
    handleMoveWidget, handleDeleteWidget, handleCloneWidget, handleWidgetUpdate, handleRenameSave, handleAddWidget
  };
}

interface HeaderProps {
  title: string; isEditingTitle: boolean; titleInput: string; loading: boolean; isDropdownOpen: boolean; isEditMode: boolean;
  onTitleClick: () => void; onTitleChange: (val: string) => void; onTitleBlur: () => void; onDropdownToggle: () => void; 
  onToggleEditMode: () => void; onAddWidget: (type: WidgetType) => void;
}

function WorkspaceHeader({ 
  title, isEditingTitle, titleInput, loading, isDropdownOpen, isEditMode,
  onTitleClick, onTitleChange, onTitleBlur, onDropdownToggle, onToggleEditMode, onAddWidget 
}: HeaderProps) {
  return (
    <header className="dashboard-header">
      <section className="title-container">
        {isEditingTitle ? (
          ...
          <>
            ...
            <button onClick={onTitleClick} className="btn-icon" title="Přejmenovat dashboard">✏️</button>
          </>
        )}
      </section>
      ...
      <section className="controls-section">
        <button 
          ...
          className="btn-add"
          style={{
            borderColor: isEditMode ? '#34d399' : undefined,
            color: isEditMode ? '#34d399' : undefined,
            background: isEditMode ? 'rgba(52, 211, 153, 0.12)' : undefined
          }}
        >
          {isEditMode ? '✅ Dokončit úpravy' : '✏️ Upravit dashboard'}
        </button>
              ...
              {(Object.keys(WIDGET_COMPONENTS) as WidgetType[]).map((type) => (
                <li key={type}>
                  <button onClick={() => onAddWidget(type)} className="dropdown-item-btn">
                    {/* 🚀 DOPLNĚNO 'list' VYKRANÍ PRO MENU */}
                    {type === 'stat' ? '📊 Graf' : 
                     type === 'progress' ? '🎯 Progress bar' : 
                     type === 'table' ? '📅 Tabulka' : 
                     type === 'image' ? '🖼️ Obrázek' : 
                     type === 'text' ? '📝 Textový box' : 
                     type === 'timer' ? '⏳ Odpočet času' : 
                     type === 'links' ? '🔗 Odkazy' : 
                     type === 'counter' ? '🔢 Počítadlo' : 
                     type === 'countup' ? '⏱️ Čas od milníku' : 
                     type === 'embed' ? '🌐 Webový Embed' : 
                     type === 'list' ? '📋 Seznam / ToDo' : 
                     '📣 Banner'}
                  </button>
                </li>
              ))}
...
interface CardProps {
  widget: DashboardWidget; index: number; totalWidgets: number; loading: boolean; activeMenuId: string | null; isEditMode: boolean;
...
function WidgetCard({ widget, index, totalWidgets, loading, activeMenuId, isEditMode, onMove, onDelete, onClone, onToggleMenu, onUpdate }: CardProps) {
    ...
    <article className={`widget-card ${gridClass}`}>
      <header className="widget-card-header">
        <div style={{ display: 'flex', alignItems: 'center', flex: 1, minWidth: 0, gap: '0.5rem' }}>
          {isEditMode && (
            <nav className="widget-order-controls" style={{ position: 'relative', flexShrink: 0 }}>
  ...
  return (
    ...
    >
      <WorkspaceHeader
        ...
        onTitleClick={() => state.setIsEditingTitle(true)} 
        onTitleChange={state.setTitleInput} 
        onTitleBlur={state.handleRenameSave} 
        onDropdownToggle={() => state.setIsDropdownOpen(!state.isDropdownOpen)} 
        onToggleEditMode={() => state.setIsEditMode(!state.isEditMode)}
        onAddWidget={state.handleAddWidget}
      />
        ...
        {state.dashboard.widgets.length === 0 ? (
          ...
          state.dashboard.widgets.map((widget, index) => (
            <WidgetCard
              ...
              isEditMode={state.isEditMode}
              onMove={state.handleMoveWidget} 
              onDelete={state.handleDeleteWidget} 
              onClone={state.handleCloneWidget} 
              onToggleMenu={state.setActiveMenuId} 
              onUpdate={state.handleWidgetUpdate}
            />
          ))
        )}
      </section>
